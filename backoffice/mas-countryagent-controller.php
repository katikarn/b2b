...
include("inc/connectionToMysql.php");
	...
	if(isset($_POST["submitAdd"]))	{
		...
		if($_POST['submitAdd'] == 'Insert')	{
			$sql = "INSERT INTO tb_agent_country_ms(agentcountry_code, agentcountry_name, create_datetime, create_by, update_datetime, update_by)
					VALUES ('$txbagentcountry_code','$txbagentcountry_name', NOW(), '$LoginByUser', NOW(), '$LoginByUser')";
		}else if($_POST['submitAdd'] == 'Update')	{
			$sql = "UPDATE tb_agent_country_ms SET
	...
	if (isset($_GET['hAction']))   {
        $sql = "DELETE FROM tb_agent_country_ms WHERE agentcountry_id = '".$_GET['id']."'";
        $result = mysqli_query($_SESSION['conn'] ,$sql);
        if(!$result) {
